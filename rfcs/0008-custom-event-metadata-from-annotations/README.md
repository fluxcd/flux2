...
After this change, there would be four sources of metadata being sent by notification-controller
to the providers. They are listed below with the proposed order of precedence, from lowest to highest:

1. User-defined metadata on the alert (set with `.spec.eventMetadata`)
2. User-defined summary on the alert (set with `.spec.summary`)
3. User-defined metadata on objects (set with the `event.toolkit.fluxcd.io/` prefix in the event payload)
4. Controller metadata (set with the `<controller group>.toolkit.fluxcd.io/` prefix in the event payload)

Upon any key conflicts when combining all the metadata above, notification-controller would
resolve them according to the precedence order specified above, print an `info` log and emit a
Kubernetes Event containing all the key conflicts to inform the user and prompt them to change
their configuration to remove these conflicts.

The API Group `event.toolkit.fluxcd.io` would be introduced as a constant in the package
`github.com/fluxcd/pkg/apis/event` and used in the package `github.com/fluxcd/pkg/runtime/event`
and notification-controller for the implementation described above.
