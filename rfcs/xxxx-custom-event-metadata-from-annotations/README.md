...
API Group in addition to those already sent by the controller. For example:
...
metadata:
  name: podinfo
  annotations:
    event.toolkit.fluxcd.io/myCustomMetadata: myValue
    event.toolkit.fluxcd.io/anotherCustomMetadata: anotherValue
```

In this example, the metadata sent to the notification providers would be:

```json
{
  "myCustomMetadata": "myValue",
  "anotherCustomMetadata": "anotherValue"
}
```
