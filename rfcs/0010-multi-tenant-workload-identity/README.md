...
// WithProvider sets the provider. Useful for tests.
func WithProvider(provider interfaces.Provider) Option {
	// ...
}

// WithProviderName sets the provider by name looking up the providers map.
...
func (t *TokenStore) GetToken(
	...
	// 1. Check if the provider is nil and return an error if it is.
	...
	// 5. Build the cache key using the ServiceAccount reference, provider identity and options.
...
// WithEndpoint sets the endpoint for the STS service.
func WithEndpoint(endpoint string) Option {
	// ...
}

// WithProxyURL sets an *http.Client configured with an HTTP/S proxy for
// talking to the STS service.
...
* The cloud provider identity, i.e. a string representing the identity which
  the `ServiceAccount` is impersonating. When there is no identity configured
  for impersonation, the string should be `interfaces.IdentityDirectAccess`.
* The optional scopes added on the token. Only Azure requires scopes.
* The optional STS endpoint.
* The optional proxy URL when the STS endpoint is specified.
...
the identity at the time when the token was issued. This is very important.
Suppose we include only the identity. Then a malicious actor could specify any
...
created, and a malicious actor could also use permissions that belong to the previous
identity.

The scopes are included in the cache key because they delimit the permissions that
the token has. They don't *grant* the permissions, they just set an upper bound on
the permissions that the token can have. Providers requiring scopes unfortunately
benefit less from caching, e.g. a token issued for an identity can't be seamlessly
used for both Azure DevOps and the Azure Container Registry, because the respective
scopes are different, so the issued tokens are different.
...
only when the STS endpoint is specified, because all the default endpoints are HTTPS
and belong to cloud providers, so they are well-known and unique.
...
attacker could still use the token for a while.

There are a few mitigations for such an attack:
...
When reconciling an object, the controller must create a `workloadidentity.TokenStore`
passing a `controller-runtime` client that has permission to create `ServiceAccount`
tokens in the Kubernetes API, the desired cloud provider by name, and the optional
cache. The provider names match the ones used for `spec.provider` in the Flux APIs,
i.e. `aws`, `azure` and `gcp`.

After creating the `TokenStore`, the controller must call `TokenStore.GetToken()`
passing the `ServiceAccount` reference from the object being reconciled and the
remaining options. The token returned by this method must be used to authenticate
with the cloud provider.

Because different cloud providers have different ways to represent the access tokens,
e.g. Azure and GCP tokens are a single opaque string while AWS tokens are a pair
(access key ID and secret access key), instances of the `interfaces.Token`
interface would need to be cast to concrete instances of `*<provider>.Token`
and used accordingly.

For this to work, it's best if the internal interfaces we use for consuming the
cloud services received all the options they need for:

* Creating the `TokenStore` specifying the appropriate provider name.
* Understanding the `*<provider>.Token` structure and using the token accordingly.

The following subsections show examples of how this integration would look like.
...
#### `OCIRepository`, `ImageRepository` and `HelmRepository` APIs

<!-- TODO: for aws, azure and gcp -->

<!-- HelmRepository only supports the spec.provider field when spec.type is oci,
so all these APIs only support container registries, hence the same underlying
auth logic -->
...
spec:
  ...
  provider: aws
  serviceAccountName: tenant-a-s3-sa
```

This `Bucket` object would cause the internal `minio.MinioClient` of source-controller
to be created with the following new options:

* `minio.WithTokenClient(controller-runtime/pkg/client.Client)`
* `minio.WithTokenCache(*fluxcd/pkg/cache.TokenCache)`

The constructor would then create a `workloadidentity.TokenStore` and use it to
get the cloud provider access token. When doing so, the `minio.MinioClient` would
cast the token interface returned by `TokenStore.GetToken()` to `*aws.Token` and
feed it to `credentials.NewStaticV4()` from the package
...
This `Bucket` object would cause the internal `gcp.GCSClient` of source-controller
to be created with the following new options:
...
The constructor would then create a `workloadidentity.TokenStore` and use it to
get the cloud provider access token. When doing so, the `gcp.GCSClient` would
cast the token interface returned by `TokenStore.GetToken()` to `*gcp.Token` and
feed it to `oauth2.StaticTokenSource()` from the package `golang.org/x/oauth2`,
then feed this token source to the `option.WithTokenSource()` and pass it to
...
The cloud provider types i.e. the values of `spec.type` that relate to one
of the cloud providers would then use these options. See the following
subsections for examples.

##### Type `azureeventhub`

Consider the following example for the `Provider` API:

```yaml
apiVersion: notification.toolkit.fluxcd.io/v1beta3
kind: Provider
metadata:
  name: tenant-a-azure-eventhub
  namespace: tenant-a
spec:
  ...
  type: azureeventhub
  serviceAccountName: tenant-a-azure-eventhub-sa
```

The `notifier.NewAzureEventHub()` constructor would use the existing and new
options to create a `workloadidentity.TokenStore` and use it to get the cloud
provider access token. When doing so, the `notifier.AzureEventHub` would cast
the token interface returned by `TokenStore.GetToken()` to `*azure.Token` and
feed the token string to `newJWTHub()`.
...
the token interface returned by `TokenStore.GetToken()` to `*azure.Token` and
feed the token string to `NewPatConnection()` from the package
...
options to create a `workloadidentity.TokenStore` and use it to get the cloud
provider access token. When doing so, the `notifier.GooglePubSub` would cast
the token interface returned by `TokenStore.GetToken()` to `*gcp.Token` and
feed it to `oauth2.StaticTokenSource()` from the package `golang.org/x/oauth2`,
then feed this token source to the `option.WithTokenSource()` and pass it to
`cloud.google.com/go/pubsub.NewClient()`.
