...
interact with cloud providers, including the source-controller,
...
the object, and exchange it for a short-lived access token for the cloud provider.
This would require the controller to have RBAC permission to create tokens for any
...
> from the Amazon ECR registry belonging to tenant A, but only from this
> registry. At the same time, I want to allow tenant B to pull OCI artifacts
> from the Amazon ECR registry belonging to tenant B, but only from this
> registry.
...
the first eliminates operational burden for users. In fact, native
...
behavior would be the same as today, i.e. the feature only activates when the field
is present.
...
As of today, not all cloud providers support custom STS endpoints, so this field
would be implemented only for the cloud providers that support it. This field
is notably already present in the `Bucket` API, and is used for both EKS
single-tenant workload identity in the `aws` bucket provider, and for LDAP
authentication in the `generic` bucket provider.
...
kustomize-controller and helm-controller do not need this feature, and
a significant amount of dependencies for talking to cloud providers would
be pulled in unnecessarily.
...
func (t *TokenStore) GetToken(
	...
	// 2. Get the ServiceAccount using the configured controller-runtime client.
	// 3. Get the identity for the ServiceAccount from the provider.
	// 4. Build the cache key using the ServiceAccount reference, provider name and identity.
	// 5. Get the token from the cache. If present, return it, otherwise continue.
	// 6. Create a token source from the provider.
	// 7. Get the audience for the OIDC token from the provider.
	// 8. Create an OIDC token for the ServiceAccount in the Kubernetes API using the audience.
	// 9. Create a new access token for the ServiceAccount using the OIDC token, add it to the cache and return it.
}
```
...
func IdentitySessionName(sa *corev1.ServiceAccount) string {
	return fmt.Sprintf("%s.%s.serviceaccounts.fluxcd.io", sa.Name, sa.Namespace)
}
...
type Provider interface {
	...
	// ServiceAccount will impersonate. When there is no identity configured for
	// impersonation, this method should return IdentityDirectAccess.
	GetIdentity(sa *corev1.ServiceAccount) (string, error)

	// NewSource returns a new Source for the provider. It should be called
	// only when it's known that a new token is needed. For example, if a
	// valid token is currently available e.g. from a cache, this method
	// should not be called.
	NewSource(ctx context.Context) (Source, error)
}

// Source contains the logic to retrieve an access token for a cloud
// provider from a ServiceAccount (OIDC/JWT) token.
type Source interface {
	// GetAudience returns the audience for creating OIDC tokens for
	// ServiceAccounts.
	GetAudience() string
	...
	// the JWT token that was issued to the ServiceAccount by the Kubernetes API.
	// The implementation should exchange this token for a cloud provider access token.
...
// Apply applies the given slice of Option(s).
...
fetched for that `ServiceAccount`.
